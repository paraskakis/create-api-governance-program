...
description: Designs an API governance program using a maturity model of five dimensions (standards, design review, automation, lifecycle, metrics) scored from evidence, committing to one level of movement rather than an aspirational end state. Emmanuel Paraskakis's method. Use when the user says "/create-api-governance-program" or asks about API governance, rolling out an API style guide, setting up an API design review process, standing up an API council or centre of excellence, enforcing standards across teams, or getting consistency across APIs built by different squads. Use it even when they never say "governance". Phrases like "every team invents their own pagination and we can't stop it", "our APIs are all inconsistent" and "we wrote standards and nobody follows them" are all this skill. Produces a markdown scorecard and a one-increment plan. Does not review an individual OpenAPI spec (that's `design-api-review`) and does not design an API (that's `design-api`).
license: MIT
metadata:
  version: "1.0.0"
